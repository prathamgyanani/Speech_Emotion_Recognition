...
get_ipython().system('pip install numpy==1.23.0')
get_ipython().system('pip uninstall numba')
get_ipython().system('pip install -U numba')
...
get_ipython().system('pip install librosa==0.9.1')
...
'''def extract_feature(file_name, mfcc, chroma, mel):
    ...
    return result
def extract_feature(file_name, mfcc, chroma, mel):
    #print(file_name,mfcc,chroma,mel)
    with soundfile.SoundFile(file_name) as sound_file:
        #print(sound_file)
        ...
        if chroma:
            chroma=np.mean(librosa.feature.chroma_stft(S=stft, sr=sample_rate).T,axis=0)
            #print("jaddu 2",chroma)
            result=np.hstack((result, chroma))
            #print("shohsa 2",result)
        if mel:
            mel=np.mean(librosa.feature.melspectrogram(y=X, sr=sample_rate).T,axis=0)
            #print("jaddu 2",mel)
            result=np.hstack((result, mel))
            #print("shohsa 3",result)
    return result'''
def extract_feature(file_name, mfcc, chroma, mel):
    with soundfile.SoundFile(file_name) as sound_file:
        if sound_file.channels > 1:
...
y_pred=Emotion_Voice_Detection_Model.predict()
y_pred=Emotion_Voice_Detection_Model.predict("C:\Users\gypra\Downloads\Speech_Emotion_Detection-master\Speech_Emotion_Detection-master\clean_speech2\03-01-08-01-02-02-24.wav")
y_pred
...
#predicting :
file="C:\\Users\\gypra\\Downloads\\Speech_Emotion_Detection-master\\Speech_Emotion_Detection-master\\clean_speech\\03-01-08-02-02-01-11.wav"
feature=extract_feature(file, mfcc=True, chroma=True, mel=True)
a=[]
a.append(feature)
